package org.openmrs.module.kenyadq.reporting.cohort.definition.evaluator;
...
public class RDQACohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	private SampleSizeConfiguration getSampleConfiguration(){
		...
		String sampleSizeConf = Context.getAdministrationService().getGlobalProperty(DqMetadata.RDQA_DEFAULT_SAMPLE_CONFIGURATION);
	...
	protected Map<Double, Integer> makePatientDataMapFromSQL(String sql, Map<String, Object> substitutions) {
		List<Object> data = Context.getService(KenyaDqService.class).executeSqlQuery(sql, substitutions);
